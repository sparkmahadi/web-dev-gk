...
const QuizDetails = ({ questionData, idx, handleClick }) => {
    ...
    return (
        <div className='p-6 border rounded-lg text-white shadow-sm my-nav shadow-xl'>
                ...
                <div className='grid grid-cols-2 gap-2 lg:px-44'>
                    {
                        options.map((option, idx) =>
                            <div key={idx} onClick={() => handleClick(option, correctAnswer)} className='border border-dark hover:bg-sky-500 p-2 m-2 rounded-lg flex items-center'>
